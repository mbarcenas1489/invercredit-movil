...
class CargarDatos : AppCompatActivity() {
  ...
  fun actualizarDatos() {
    ...
    clearDataBase()
    CargarDatosService.ObtenerCreditos(
      binding.constracionCargarDatos,
      binding.tvCreditosPorcentaje,
      binding.tvCreditosCantidad
    )
    CargarDatosService.ObtenerCuotas(
      binding.constracionCargarDatos,
      binding.tvCuotasPorcentaje,
      binding.tvCuotasCantidad
    )
    PlanesPagoService.ObtenerPlanesPagoPorCobrador(
      binding.constracionCargarDatos,
      binding.tvPlanesPagoPorcentaje,
      binding.tvPlanesPagoCantidad
    )
    CargarDatosService.ObtenerMoras(
      binding.constracionCargarDatos,
      binding.tvMorasPorcentaje,
      binding.tvMorasCantidad
    )
    CobradorService.ObtenerCobradores(
      binding.constracionCargarDatos,
      binding.tvCobradoresPorcentaje,
      binding.tvCobradoresCantidad
    )
