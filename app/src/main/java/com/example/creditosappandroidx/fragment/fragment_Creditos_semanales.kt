...
class fragment_Creditos_semanales : Fragment() {

  private lateinit var binding: FragmentCreditosSemanalesBinding
  ...
  override fun onCreateView(
    ...
    binding = FragmentCreditosSemanalesBinding.inflate(layoutInflater)
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    binding.searchViewCreditoSemanal.queryHint = "Buscar Client"
    if (BuildConfig.BUILD_TYPE == "admin") {
      datospublicoskt.ObtenerCreditoPorCobrador(view.context, 2)
    } else {
      datospublicoskt.cargarlistacredito(view.context, 2)
    }
    ...
    binding.searchViewCreditoSemanal.setOnQueryTextListener(object :
      ...
      override fun onQueryTextChange(newText: String?): Boolean {
        if (newText != null) {
          ...
          var adap = adaptador_cliente_semanal(view.context, datospublicoskt.listacompleta)
          if (adap != null) {
    ...
    binding.listviewClienteSemanal.setOnItemClickListener { parent, view, position, id ->
  ...
  fun getcuotaporsemana(adap: adaptador_cliente_semanal?) {
    GlobalScope.launch(Dispatchers.IO) {
      ...
      datospublicoskt.listacompleta.forEach({
        ...
        {
          if (binding.searchViewCreditoSemanal?.query.toString().length > 0) {
            text = binding.searchViewCreditoSemanal?.query.toString();
            filter()
          } else
  ...
  fun filter() {
    ...
    if (charText.length == 0 && listafiltrada.size == 0) {
      ...
      var adap = adaptador_cliente_semanal(context, datospublicoskt.listacompleta)
      if (adap != null) {
        adap.notifyDataSetChanged()
      }
      return
